	...
	{	
		...
		private function playNext(me:MediaEvent = null):void
		{
			...
			if (_currentPlaylistIndex > 1)
				_toolsLoader.container.viewController.callModelMethod(PluginMethods.SET_PREV_BUTTON_VISIBILITY, true);
			else	
				_toolsLoader.container.viewController.callModelMethod(PluginMethods.SET_PREV_BUTTON_VISIBILITY, false);
				
			if (_currentPlaylistIndex < _endIndex)	
				_toolsLoader.container.viewController.callModelMethod(PluginMethods.SET_NEXT_BUTTON_VISIBILITY, true);
			else	
				_toolsLoader.container.viewController.callModelMethod(PluginMethods.SET_NEXT_BUTTON_VISIBILITY, false);
		}
