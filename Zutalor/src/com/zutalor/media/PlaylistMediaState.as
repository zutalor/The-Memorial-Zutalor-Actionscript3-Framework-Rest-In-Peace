...
{	
	...
	public class PlaylistMediaState extends EventDispatcher implements IDisposable
	{
		...
		public function initialize(sp:MediaStateProperties, players:gDictionary):void
		{
			...
			_pp = _ppm.getPropsById(sp.playlist.playlistName);
		...
		private function getPlayer(pip:PlaylistItemProperties, cueing:Boolean):*
		{
			...
			{
				if (!pip.width)
					p.load(fullUrl, pip.volume, _sp.playlist.width, _sp.playlist.height, _pp.scaleToFit, _pp.bufferTime);
				else	
					p.load(fullUrl, pip.volume, pip.width, pip.height, _pp.scaleToFit, _pp.bufferTime);
			}	
			return p;
		}
